# ===============================
# PIPELINE R: RoBERTa + BiLSTM + SVM (FIXED)
# ===============================
...
if os.path.exists(embedding_path):
    ...
    print(f"⚠️ Embeddings not found at '{embedding_path}'. Generating new RoBERTa embeddings...")
    
    # 1. Load RoBERTa tokenizer and model
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(texts), EXTRACT_BATCH_SIZE), desc="Generating RoBERTa Embeddings"):
...
for fold, (train_idx, test_idx) in enumerate(outer_cv.split(X_seq_raw, y)):
    ...
    # Added dual="auto" for new sklearn standard
    grid = GridSearchCV(
...
# Pass the true unbiased F1-Macro to evaluate_and_save
grid_dummy = DummyGrid(best_params_final, true_f1_macro, param_grid)

evaluate_and_save(
    "PIPELINE_R_ROBERTA_BILSTM_SVM",
    grid_dummy,
    best_model,
    X_train_feat,
    y,
