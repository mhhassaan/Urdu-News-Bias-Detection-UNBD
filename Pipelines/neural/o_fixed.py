...
# PIPELINE O: LaBSE + LSTM + LOGISTIC REGRESSION (NO LEAKAGE)
import os
import re
import numpy as np
import pandas as pd
...
from sklearn.metrics import f1_score # Added for true metric calculation

from evaluate_and_save import evaluate_and_save

print("\n===== PIPELINE O: LaBSE + LSTM + LOGISTIC REGRESSION (FIXED) =====")
...
class LSTMModel(nn.Module):
    ...
    def forward(self, x, return_features=False):
        ...
        features = torch.mean(output, dim=1)  # ✅ mean pooling
...
for fold, (train_idx, test_idx) in enumerate(outer_cv.split(X_seq_raw, y)):
    ...
    for epoch in range(num_epochs):
        ...
        loop = tqdm(range(0, len(X_tensor), batch_size), desc=f"Fold {fold+1} | Epoch {epoch+1}/{num_epochs}", leave=False)
    ...
    # These predictions are completely unbiased!
...
best_params_final = max(set(map(str, best_params_list)), key=list(map(str, best_params_list)).count)
best_params_final = eval(best_params_final)

grid_dummy = DummyGrid(best_params_final, true_f1_macro, param_grid)

evaluate_and_save(
    "PIPELINE_O_LABSE_LSTM_LR_FIXED",
    grid_dummy,
    best_model,
    X_train_feat,
    y,
