...
if os.path.exists(embedding_path):
    ...
    X_labse = np.load(embedding_path)
else:
    raise FileNotFoundError("Run Pipeline E first.")
...
for fold, (train_idx, test_idx) in enumerate(outer_cv.split(texts, y)):
    ...
    X_train_labse = X_labse[train_idx]
    X_test_labse = X_labse[test_idx]
    ...
    # COMBINE FEATURES
    X_train = np.hstack((X_train_tfidf, X_train_labse))
    X_test = np.hstack((X_test_tfidf, X_test_labse))
